...
const ClientFormPage = ({ t, lang }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch("https://script.google.com/macros/s/AKfycbwicOUQUUkEJ-fZ4z4TVo5rJqQclMPqjIiqrYbx8bZC-DKmO-qR9k0gCgznimIY1Glk/exec", {
        method: "POST",
...
const CoachDashboard = ({ onLogout, t, lang }) => {
  ...
  useEffect(() => {
    fetch("https://script.google.com/macros/s/AKfycbwicOUQUUkEJ-fZ4z4TVo5rJqQclMPqjIiqrYbx8bZC-DKmO-qR9k0gCgznimIY1Glk/exec")
      .then(r => r.json())
      .then(data => {
        setClients(data);
        const p = {};
              ...
              <div className="card">
                ...
                {selected.phone && (
                  <button className="ghost-btn" style={{ width: "100%", padding: 14, marginTop: 8, background: "#25D366", color: "#fff", border: "none" }}
                    onClick={() => {
                      const phone = String(selected.phone).replace(/[^0-9]/g, "");
